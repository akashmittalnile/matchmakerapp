...
const ShopCart = (props) => {
  ...
  const getcart = async () => {
    // setresData([])
    console.log("business_id111111",VenderDetails);
    setLoading(true)
    const { responseJson, err } = await requestGetApi(shop_eat_cart + '?business_id='+ VenderDetails.business_id, '', 'GET', User.token)
    setLoading(false)
  ...
  const applyCoupan = async () => {
    if (promocode == null || promocode == '') {
      ...
      var data = {
        ...
        business_id:VenderDetails.business_id
