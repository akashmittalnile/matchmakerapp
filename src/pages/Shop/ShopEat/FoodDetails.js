...
const FoodDetails = (props) => {
  ...
  const menuList = async (dd) => {
    ...
    if (dd == null) {
      ...
      urls = shop_eat_menu + props.route.params.data.userid + '?menu_type=' + dd + '&service_type=' + resData.services?.find(el => el.attribute_label == selectedTab)?.attribute_code
      // urls=shop_eat_menu_userid+props.route.params.data.userid
    }
    ...
    if (responseJson.headers.success == 1) {
      ...
      if (responseJson.body.length > 0) {
        // console.log('menuList responseJson.body', responseJson.body);
        // Toast.show({ text1: responseJson.headers.message });
        setmenuresData(responseJson.body)
      } else {
        // console.log('menuList NULL Array', responseJson.body);
        setmenuresData([])
      }
        ...
        <View style={{ marginLeft: 15 }}>
          ...
          <View style={{ marginTop: 5, backgroundColor: '#fff', alignItems: 'flex-start', flexDirection: 'row' }}>
            ...
            <Rating
              ...
              readonly={true}
            />
            <Text style={{ color: 'gray', fontSize: 11, top: -2 }}> {Number(item.total_ratings).toFixed(0)} Ratings</Text>
          </View>
  ...
  const getTakeAwayData = () => {
    console.log('getTakeAwayData', menuresData);
    return (
      <>
        {
  ...
  const getDeliveryData = () => {
    console.warn("menuresDatamenuresData", menuresData);
    return (
      <>
        {
          menuresData.map((item, index) => {
          ...
          <View style={{ width: '96%', alignSelf: 'center', backgroundColor: '#F5F5F5', marginHorizontal: 20, borderRadius: 10, justifyContent: "center", top: -25, }}>
            <FlatList
              ...
              renderItem={({ item, index }) => {
                console.log('TAB click FlatList', selectedValue);
                return (
                  <>
                    {item.attribute_value == 'yes' ?
                      <View style={{ width: dimensions.SCREEN_WIDTH * 40 / 100, marginHorizontal: 8, padding: 5 }}>
                        <MyButtons title={item.attribute_label} height={37} width={'100%'} borderRadius={5} alignSelf="center"
                          press={() => {
                            setselectedTab(item.attribute_label)
                            setselectedValue('')
                            setmenutypevalue('')
                            menuList()
